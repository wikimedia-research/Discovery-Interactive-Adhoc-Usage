...
Let's count how many articles on those wikis have mapframes...
...
Below are the results as of 13 July 2017:
...
One way to consider usage is how many pages are geo-tagged vs not (query run on 13 July 2017):

```{sql geo_tags, connection=con, cache=TRUE}
SELECT
  COUNT(*) AS `total files`,
  SUM(is_geotagged) AS `geo-tagged files`,
  SUM(is_geotagged)/COUNT(*) AS `proportion geo-tagged`
FROM (
  SELECT
